<?php if (!defined('THINK_PATH')) exit(); if(is_array($tree)): $i = 0; $__LIST__ = $tree;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><dl class="cate-item">
		<dt class="cf">
			<form action="<?php echo U('edit');?>" method="post">
				<input type="hidden" name="is_page" value="<?php echo ($list["is_page"]); ?>">
				<input type="hidden" name="allow_publish" value="<?php echo ($list["allow_publish"]); ?>">
				<div class="btn-toolbar opt-btn cf">
					...
					<?php if(($list["is_page"]) != "1"): ?><a title="移动" href="<?php echo U('operate?type=move&from='.$list['id']);?>">移动</a>
					<a title="合并" href="<?php echo U('operate?type=merge&from='.$list['id']);?>">合并</a><?php endif; ?>
				</div>
				...
				<div class="name">
					...
					<?php if(($list["is_page"]) != "1"): ?><a class="add-sub-cate" title="添加子分类" href="<?php echo U('add?pid='.$list['id']);?>">
							<i class="icon-add"></i>
						</a><?php endif; ?>
