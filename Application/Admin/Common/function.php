...
/**
 * 添加模型时候 字段描述规则 以;开头表示，多个模型不同字段描述， ;模型id:描述信息|模型id:描述信息|模型id:描述信息
 * 获取不同模型字段的不同描述信息
 * @param $fieldTitle
 * @param $model_id
 * @return mixed
 */
function getModelFieldDesc($fieldTitle, $model_id)
{
    if(substr($fieldTitle, 0, 1) == ';') {
        $fieldTitle = substr($fieldTitle, 1);
        $title_arr = str2arr($fieldTitle, '|');

        $title = $title_arr[0];
        foreach($title_arr as $key=>$value) {
            $v = str2arr($value, ':');
            $new_val[$v[0]] = $v[1];
            if(isset($new_val[$model_id])) {
                $title = $new_val[$model_id];
                break;
            }
        }
        if($title == '') {
            $v = str2arr($title_arr[0], ':');
            $title = $v[1];
        }
    } else {
        $title = $fieldTitle;
