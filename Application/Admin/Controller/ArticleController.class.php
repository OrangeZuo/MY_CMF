...
class ArticleController extends AdminController {
    ...
    protected function getMenu(){
        ...
        $cate       =   M('Category')->where(array('status'=>1))->field('id,title,pid,allow_publish,is_page')->order('pid,sort')->select();
        ...
        //生成每个分类的url
        foreach ($cate as $key=>&$value){
            $value['url']   =   (0 == $value['is_page']) ? 'Article/index?cate_id='.$value['id'] : 'Article/page?cate_id='.$value['id'];
            ...
            if(!empty($value['_child'])){
                ...
                foreach ($value['_child'] as $ka=>&$va){
                    $va['url']      =   (0 == $va['is_page']) ? 'Article/index?cate_id='.$va['id'] : 'Article/page?cate_id='.$va['id'];
                    if(!empty($va['_child'])){
                        foreach ($va['_child'] as $k=>&$v){
                            $v['url']   =   (0 == $v['is_page']) ? 'Article/index?cate_id='.$v['id'] : 'Article/page?cate_id='.$v['id'];
                            $v['pid']   =   $va['id'];
     ...
     * @author 范思宇
     */
    public function page($cate_id = null){
           ...
           $catid = I('catid');
            ...
            if(false !== $model->create()){
                ...
                if( $model->save()){
                    ...
                    $this->success('保存成功！', U('page',array('cate_id'=>$catid)));
            ...
            $page = $model->where('catid='.$cate_id)->find();

            $this->assign('p',$page);

            $this->meta_title = '单文章编辑';
