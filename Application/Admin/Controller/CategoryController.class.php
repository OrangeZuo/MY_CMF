...
class CategoryController extends AdminController {
    ...
    public function index(){
        $tree = D('Category')->getTree(0,'id,name,title,sort,pid,allow_publish,is_page,status');
    ...
    public function edit($id = null, $pid = 0){
        ...
        if(IS_POST){ //提交表单
            ...
            $isPage = I('post.is_page');
            $allowPublish = I('post.allow_publish');
            $updateId = $Category->update();

            if(false !== $updateId ){

                if(1 == $isPage && 0 !=$allowPublish){
                    $Page =  D('Page');
                    $data['catid']=$id;
                    $isAdd = $Page->where('catid='.$id)->find();
                    $data['title'] = I('post.title', '');
                    $data['keywords'] =  I('post.keywords', '');
                    $data['description'] =  I('post.description', '');
                    ($isAdd) ? $Page->save($data) : $Page->add($data);
                }
    ...
    public function remove(){
        ...
        M('Page')->where(array('catid'=>$cate_id))->delete();
        //删除该分类信息
