...
class IndexController extends HomeController {
    ...
    public function index() {
        ...
        $link_l_pic    = D('document')->where('status = 1 and category_id = 116')->select();
        $link_r_pic    = D('document')->where('status = 1 and category_id = 117')->find();
        $user_say_pic  = D('document')->where('status = 1 and category_id = 115')->find();
        $friend_link  = D('document')->where('status = 1 and category_id = 118')->select();
        ...
        $this->assign('link_l_pic', $link_l_pic);               // 友情链接左边图
        $this->assign('link_r_pic', $link_r_pic);               // 友情链接右边图
        $this->assign('user_say_pic', $user_say_pic);           // 用户反馈图
        $this->assign('friend_link', $friend_link);           // 友情链接
